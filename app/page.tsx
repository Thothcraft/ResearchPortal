...
import { Camera, Radar, Activity, ShieldCheck } from 'lucide-react';
...
export default function PortalEntry() {
  ...
  const sensing = [
    { title: 'Radar & CSI', text: 'Presence, motion, and channel-state sensing without identifiable imagery.', icon: Radar },
    { title: 'IMU & environmental', text: 'Vibration, acceleration, temperature, humidity, and pressure.', icon: Activity },
    { title: 'Optional camera', text: 'Visual context when your deployment calls for it.', icon: Camera },
  ];
  const uses = ['Smart home', 'Health & elder care', 'Security & intrusion', 'Industrial & vibration', 'Research & datasets'];
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <section className="mx-auto grid min-h-[68vh] max-w-6xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.2fr_.8fr]">
      <div><div className="text-xs font-bold uppercase tracking-[.2em] text-cyan-700">Thothcraft programmable sensing</div><h1 className="mt-5 max-w-4xl text-5xl font-semibold leading-[.95] tracking-[-.055em] sm:text-7xl">Program any space with multimodal sensing.</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-slate-600">Thoth nodes run the Whispy SDK—fusing radar, CSI, IMU, environmental, and optional camera sensing—then link to Brain for fleet management, models, and captures.</p><Link href="/auth" className="mt-8 inline-flex rounded-xl bg-cyan-300 px-5 py-3 font-semibold text-slate-950">Open thothHUB</Link></div>
