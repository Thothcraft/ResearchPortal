...
export default function CaptureViewerPage() {
  ...
  const expectedChunks = Number(manifest?.expected_chunks || 0) || Math.max(predictionTimeline.length, 1);
  ...
  return <div className="space-y-6 text-slate-950">
    ...
    <section className="border border-slate-300 bg-white p-4"><div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">Chunk predictions</h2><span className="text-xs text-slate-500">{predictionTimeline.length} / {expectedChunks} analyzed</span></div><div className="flex items-start gap-2">{Array.from({ length: expectedChunks }, (_, index) => { const entry: any = predictionByIndex.get(index); const state = entry ? (entry.occupied === true ? 'occupied' : 'empty') : 'waiting'; const color = state === 'occupied' ? 'bg-emerald-500' : state === 'empty' ? 'bg-slate-500' : 'bg-slate-200'; return <div key={index} className="min-w-0 flex-1 text-center" title={`Chunk ${index + 1}: ${state}`}><div className={`mx-auto h-3 w-3 rounded-full ${color}`} /><div className="mt-1 truncate text-[9px] font-semibold uppercase text-slate-600">{state}</div><div className="mt-1 text-[9px] text-slate-400">{Math.round(index * Number(predictions?.chunk_seconds || manifest?.chunk_seconds || 10))}s</div></div>; })}</div></section>
